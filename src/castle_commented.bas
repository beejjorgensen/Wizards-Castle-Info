...
1140 IF A-X + B-Y = 0 THEN PRINT "** TURKEY! THAT'S NOT A DIRECTION": GOTO 620
...
1220 ST = ST - FNA(3): PRINT "WEAKER": ON 1 - (ST < 1) GOTO 620, 2480
1230 IQ = FNC(IQ + FNA(3)): PRINT "SMARTER": GOTO 620
1240 IQ = IQ - FNA(3): PRINT "DUMBER": ON 1 - (IQ < 1) GOTO 620, 2480
1250 DX = FNC(DX + FNA(3)): PRINT "NIMBLER": GOTO 620
1260 DX = DX - FNA(3): PRINT "CLUMSIER": ON 1 - (DX < 1) GOTO 620, 2480
...
1470 POKE FND(Z),1: T = T + 20: O$ = MID$("NSWE", FNA(4), 1): GOTO 950
...
1500 PRINT "YOURSELF IN A BLOODY HEAP": ST = ST - FNA(2): ON 1 - (ST < 1) GOTO 1420, 2840
...
1650 IF O$ <> "Y" THEN PRINT "** THEN DON'T SAY THAT YOU DO": GOTO 620
